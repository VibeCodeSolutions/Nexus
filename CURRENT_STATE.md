...
**Stand:** 2026-05-19
**Aktuelle Phase:** v0.1.3 released + Post-Release-Stack abgeschlossen: NV-Closure + FEAT-001 KI-Aufgabensplitting + VC-013-VOL Settings-Toggle + FEAT-002 (A iCal-Export, B/C Härtung) + NV-Vision-clippy-Cleanup + Daniel-Funktional Sprint + Daniel-Polish Sprint (Listen-/Cards-/Detail-Polish + Foto-Pipeline-Polish, Desktop+Android) + S24-Smoke-Polish (4 Findings aus erstem S24-Native-Smoke) + **S24-VISION-FIX (5 Phasen, Vision-Provider-Stack: Ollama-Closure + Anthropic Claude + Gemini + OpenAI-kompatibel für openai/openrouter/xai/mistral)**. Kein aktiver Sprint, S24-Projekte-CRUD steht als nächster Sprint angemeldet.
**Phase-Status:** v0.1.0 GA, v0.1.2 + v0.1.3 released. Letzter Commit `77a9a39` (fix(s24-vision-fix-p5-hotfix) Ollama-Tags uppercase). Branch **`design-system/v0.2-pulse`** — Vision-Stack-Commits liegen auf Feature-Branch, Merge-Strategie nach main offen (Admin-Entscheidung pendent). `cargo clippy --all-targets -- -D warnings` grün. Tests 132/0+1ign (+19 vs. Vorlauf). assembleDebug+lintDebug — Android-Pfad nicht berührt durch diesen Sprint, letzter Stand grün.

**S24-VISION-FIX-Details:** 5 atomare Commits auf `design-system/v0.2-pulse`. P1 `3ede4bc` Ollama-Closure (mod ollama; + Provider-Switch-Branch + VisionConfig.vision_model "llava" + enabled-Default-Flip false→true). P2 `58c6423` Anthropic Claude Vision (claude.rs neu, Messages-API mit base64-image-Source, default claude-haiku-4-5-20251001). P3 `f325f7a` Gemini Vision (gemini.rs neu, generateContent mit inline_data, default gemini-2.5-flash). P4 `687a8d2` OpenAI-kompatibler Adapter (openai_compatible.rs neu, ein Struct für openai/openrouter/xai/mistral — Defaults gpt-4o-mini / anthropic/claude-haiku-4.5 / grok-2-vision-1212 / pixtral-12b-2409). P5 Sammel-QS Tuvok qs-20260518-S24VF (0 Blocker / 0 Major / 1 Minor VC-001-KON Ollama-Tag-uppercase-Konsistenz, geheilt via Hotfix `77a9a39` vor Findings-Gate). Provider-Default bleibt `groq`, alle anderen via `NEXUS_VISION_PROVIDER`-Env-Switch. Modell-Halluzinations-Check pro Default-Modell durch Tuvok bestanden.
...
**Post-v0.1.3:** NV-Closure (3 Polish-Bookmarks) ✅ · FEAT-001 KI-Aufgabensplitting (4 Schichten) ✅ · VC-013-VOL Settings-Toggle (FEAT-001-C-Auflage) ✅ · FEAT-002-A iCal-Export Sparks+Tasks ✅ · FEAT-002-B/C iCal-Härtung (P1 AUTH+TRACE, P2 ETag+Last-Modified+304) ✅ · NV-Vision-clippy-Cleanup (3 pre-existing Warnings beseitigt, strict-Modus grün) ✅ · Daniel-Funktional Sprint (P1 Core+Endpoints, P2 Desktop UI, P3 Android UI) ✅ · Daniel-Polish Sprint (P1 Listen+Cards+Detail DA-002/003/004/005+DC-005 commit `fff0657`, P2 Foto-Pipeline DB-001/002/003/005+DB-004 Desktop-only commit `753ba3a`) ✅. QS-Läufe qs-20260517-011..014 + qs-20260518-001..009 alle Freigabe oder Auflagen-Freigabe (zwei Auflagen-Schleifen in 1 Zyklus geheilt: DC-P2 + DB-001/DB-002).

**Daniel-Polish-Details:** P1 — DA-002 Kind-Badge IDEA/TASK auf Spark-Cards (clientseitiges Mapping via nexus_external_id-Regex auf Tasks-Endpoint, kein Backend-Touch), DA-003 Karten-Stagger 18px slide-up+opacity+scale (Cap nth-child(1..8)/coerceAtMost(320) gegen Lange-Listen-Wartezeit), DA-004 Tag-Pop-In im Detail-Sheet (60ms-Versatz gestaffelt), DA-005 Sheet-Dim-Overlay (Body-Klasse + `.entry-card:not(.selected)` für Desktop, graphicsLayer.alpha für Android), DC-005 Tasks-Checkbox-Animation (stroke-dasharray-Pfad `M 2 6 L 5 9 L 10 3` Desktop / Canvas+PathMeasure Android). P2 — DB-001 PreviewScanOverlay Android (4 Ecken-Marker 28dp + animierte Gradient-Linie via rememberInfiniteTransition Reverse-Mode), DB-002 Analyse-/Done-Status-Pill mit Spinner→Check (Desktop overlay-chip mit Backdrop-Blur + data-state-Variants, Android Surface(RoundedCornerShape(50)) + CircularProgressIndicator), DB-003 blinkender ▌-Cursor während OCR-Streaming (CSS ::after Desktop, BlinkingCursor-Composable Android), DB-004 Glow-Hover-Ring auf Desktop-CTA (4px lila + 24px Glow, Android-Skip Touch), DB-005 Shutter-Flash beim Capture (Desktop 0.15s @keyframes + Reflow-Force-Retrigger, Android animateFloatAsState 40/110ms). Worker-Spawn-Mix: P1+P2-Desktop via general-purpose Subagent, P2-Android via Lead-Direktarbeit nach Content-Filter-Block (zweimal hintereinander geblockt beim Kombi-Vokabular Camera/Capture/OCR/Image-Pipeline — Lehre: bei multimedialen Polish-Items knappe Filter-arme Briefings nutzen).
