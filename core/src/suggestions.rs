...
pub struct ProjectSuggestionRow {
    ...
    pub member_spark_ids: String, // JSON-Array as String
    pub confidence: f64,
...
pub struct ProjectSuggestionInput {
    pub name: String,
    pub description: String,
    pub member_spark_ids: Vec<String>,
    pub confidence: f64,
    pub reason: Option<String>,
}
...
pub async fn insert(pool: &SqlitePool, input: &ProjectSuggestionInput) -> Result<ProjectSuggestionRow, sqlx::Error> {
    ...
    let ids_json = serde_json::to_string(&input.member_spark_ids)
        .unwrap_or_else(|_| "[]".to_string());
    sqlx::query(
        "INSERT INTO project_suggestions (id, name, description, member_spark_ids, confidence, reason) \
         VALUES (?, ?, ?, ?, ?, ?)",
    )
    .bind(&id)
    .bind(&input.name)
    ...
    sqlx::query_as::<_, ProjectSuggestionRow>(
        "SELECT id, name, description, member_spark_ids, confidence, reason, created_at, status FROM project_suggestions WHERE id = ?",
    )
    .bind(&id)
    .fetch_one(pool)
    .await
...
pub async fn list_pending(pool: &SqlitePool) -> Result<Vec<ProjectSuggestionRow>, sqlx::Error> {
    sqlx::query_as::<_, ProjectSuggestionRow>(
        "SELECT id, name, description, member_spark_ids, confidence, reason, created_at, status \
