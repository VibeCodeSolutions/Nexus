...
- **`links`-Tabelle** + Repo-Modul `core/src/links.rs` für polymorphe Verknüpfungen zwischen Sparks und Projekten. CRUD + `delete_for_node`-Cascade-Helper (5 Inline-Tests). Migration `20260501_001_links.sql`.
...
- **7 neue Bearer-pflichtige Endpoints:** `POST /links`, `DELETE /links/{id}`, `GET /spark/{id}/links`, `GET /projects/{id}/links`, `GET /projects/suggestions`, `POST /projects/suggestions/{id}/{accept,dismiss}`.
...
- **Sentinel-Marker (`relation='noop-marker'`)** verhindert Cost-Loop bei API-LLMs: Sparks mit 0 LLM-Treffern bekommen einen Selbst-Link, sodass der NOT-EXISTS-Filter im nächsten Cycle greift. Bei `Err(...)` wird kein Sentinel geschrieben — temporäre Fehler dürfen retryen.
- **Cleanup-Cascade in `repo::delete_spark`/`delete_project`** ruft `links::delete_for_node` für polymorphe Orphan-Verhinderung.
...
- **Spark-Detail-Modal** mit Volltext, Meta (Kategorie + Datum), Summary, Tags und neuer **"Verknüpft mit"-Section**. Wikilinks 📁 für Projects, 📝 für Sparks mit Confidence-Anzeige in Prozent. Klick navigiert (rekursiv für Sparks, Tab-Switch für Projects).
...
- **Desktop UI komplett deutsch** (PC + JJ + SM): Header, Tabs, Toolbars, Modals, JS-Banner, **catch-Body-Innerhtml** (SM-F-RETRO-001 in Phase X), **Tabellen-Header** (Kategorie/Inhalt/Datum/Aktionen), Empty-States (Keine Sparks/Projekte gefunden), Status-/Priority-Mappings auf Android (Offen/Erledigt, Niedrig/Mittel/Hoch).
...
- **`accept_project_suggestion`-Response** (SM-B-006): zählt erfolgreich verknüpfte Sparks explizit, neue Felder `linked_sparks`, `requested_sparks`, `partial`-Flag.
...
- **SM-U-001 Race-Guard:** `currentBdDetailId !== id`-Check nach `await api()` in `openSparkDetail` verhindert Stale-Render bei rekursiver Wikilink-Navigation.
...
- Live-Verifikation: Theme-Switch Hell→Dunkel→System, Persistenz über App-Restart, Footer sichtbar auf allen Routes (Spark + Settings live geprüft).
...
- **`GET /spark/unsorted/count`** — cheap COUNT-Endpoint für UI-Badges.
